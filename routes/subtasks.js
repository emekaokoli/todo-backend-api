...
const ok = (res, data) => res.status(200).json(normalizeResponse(data));
  ...
  try {
    findOneSubtask({
      ...
      resolve: async (subtask) => {
        ...
        const result = await subtasksModel.updateStatus({
          id,
          status: newStatus,
          todo_id,
        });
        ...
        const isAllCompleted = allSubtasks.every(
          (st) => st.status === 'completed',
        );

        await tasksModel.updateStatus({
          id: todo_id,
